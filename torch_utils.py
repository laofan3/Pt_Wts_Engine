...
import torch
from torch import __version__, Tensor, float16, diag, sqrt, mm, zeros, no_grad
from torch.cuda import is_available, get_device_properties, synchronize, memory_reserved, empty_cache

# import torch.distributed as dist
from torch.distributed import barrier
# import torch.nn as nn
from torch.nn import Module, parallel, Conv2d, BatchNorm2d, Hardswish, LeakyReLU, ReLU, ReLU6, Parameter
...
def select_device(device='', batch_size=None):
    ...
    if cpu:
        ...
        assert is_available(), f'CUDA unavailable, invalid device {device} requested'  # check availability

    cuda = not cpu and is_available()
    if cuda:
...
def profile(input, ops, n=10, device=None):
    ...
    for x in input if isinstance(input, list) else [input]:
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            m = m.half() if hasattr(m, 'half') and isinstance(x, Tensor) and x.dtype is float16 else m
            ...
            try:
                ...
                mem = memory_reserved() / 1E9 if is_available() else 0  # (GB)
...
def model_info(model, verbose=False, img_size=640):
    ...
    try:  # FLOPs
        ...
        img = zeros((1, model.yaml.get('ch', 3), stride, stride), device=next(model.parameters()).device)  # input
